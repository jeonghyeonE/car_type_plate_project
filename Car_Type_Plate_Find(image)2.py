...
plate_model_path = 'runs/detect/plate_detection3/weights/best.pt'
...
# 베이스 폴더 경로 설정 (여기에 이미지들이 저장된 폴더 경로를 지정)
base_folder = 'D:/dataset/자동차 차종-연식-번호판 인식용 영상/원본이미지/[원본]원천데이터1'  # 이미지들이 저장된 폴더 경로

# 베이스 폴더 내 모든 이미지 파일 처리
for img_file in os.listdir(base_folder):
    if img_file.endswith(('.jpg', '.jpeg', '.png')):  # 이미지 파일 필터링
        image_path = os.path.join(base_folder, img_file)
        ...
        # 결과 이미지 저장 (원본 이미지 이름을 유지하여 결과 파일로 저장)
        # output_image_path = os.path.join(base_folder, f'output_{img_file}')
        # cv2.imwrite(output_image_path, frame)  # 결과 이미지를 저장

        # 이미지를 표시하려면 아래 코드를 사용
